...
INSTRUCTIONS = (
    "Read-only view into ComfyTV, the canvas-app layer running inside this "
    "ComfyUI. Call server_info first. Tools report state (projects, canvas, "
    "workflows, outputs, assets, jobs, errors); nothing here can run or "
    "modify anything. The canvas snapshot is mirrored from an open ComfyTV "
    "browser tab, and mirroring only activates once an MCP client connects — "
    "right after connecting, retry get_canvas after ~10 seconds before "
    "concluding no tab is open. If it stays unavailable or stale, tell the "
    "user to open (or focus) the ComfyTV page instead of guessing. For "
    "machine-level "
