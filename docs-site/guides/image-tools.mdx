...
Background removal. Wire an image, click Run, get a PNG with a real alpha channel. Ships with `BiRefNet Cutout` — requires `birefnet.safetensors` in `models/background_removal/` (see [models.md](/guides/models)).
...
For 360° tools see [panorama.md](/guides/panorama); for picking / comparing see [compose.md](/guides/compose).
