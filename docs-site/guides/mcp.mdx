...
description: "ComfyTV ships an MCP server so AI agents can read and drive your canvas: build node graphs, run stages, wait for renders, inspect results with real vision, edit the native ComfyUI graph, and manage your workflow configuration — all through 45 tools, plus installed [Agent Skills](skills.md) served as MCP prompts."
---
...
| `skill` | List installed [Agent Skills](/guides/skills) and read their instructions — the tool's own description carries a live index of what's installed |
...
**Skills.** When the user has [Agent Skills](/guides/skills) installed, the `skill` tool's description lists them; read a matching skill before acting and follow it. Each enabled skill is also exposed as an **MCP prompt** — in Claude Code they appear as `/mcp__comfytv__<skill-name>` slash commands for explicit invocation.
...
- [Agent Skills](/guides/skills) — instruction packs served through the `skill` tool and MCP prompts
