---
title: "Director"
description: "A clip timeline that renders a whole short film segment by segment: per-clip prompts, references, transitions and seeds, with content-addressed caching so only edited clips re-render."
sidebarTitle: "Director"
---

<Frame caption="Director · screenshot coming soon">
  <img src="/images/nodes/_placeholder.svg" alt="Director" />
</Frame>
...
2. Attach your cast as asset references on the node — these are `@image_1`, `@image_2`, … in every clip.
...
A: No. The pool is merged with shared references first, so `@image_1` is the first shared reference in every clip; clip-local references continue the numbering.
