---
title: "生成内容"
translationSourceHash: "2ba05d5a752efd20"
---
...
- **🔗 Link workflow** —— *使用自定义工作流的推荐方式。*直接从 ComfyUI 自己的工作流库里挑一个链接进来；在 ComfyUI 里编辑保存，ComfyTV 自动读到最新版。见 [custom-workflows.zh.md](/zh/guides/custom-workflows)。
...
出片之后，整个[视频套件](/zh/guides/video-and-audio)——剪辑、调色、抠像、合成、特效——就接手了。
...
输出是一个 FLAC 音频文件。之后[音频套件](/zh/guides/video-and-audio)（均衡、分轨、混响……）和 [Music 节点](/zh/guides/making-music)都能接手。
