...
> 这里编辑的是和 `_preset.json` 同一套绑定接口（见 [custom-workflows.zh.md](/zh/guides/custom-workflows) 里的「完整路径」）。侧边栏面向终端用户调整内置默认。
...
1. 加一个工作流——🔗 链接、⬆ 上传,或把 JSON 放进库目录(见 [custom-workflows.zh.md](/zh/guides/custom-workflows))。
...
- [custom-workflows.zh.md](/zh/guides/custom-workflows) , 怎么打包自己的工作流 + `_preset.json`
