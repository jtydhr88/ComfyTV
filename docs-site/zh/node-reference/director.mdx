...
- **共享演员表 + 逐段参考**:挂在节点上的资产参考是全片演员表(序数在每段稳定);每段还能叠自己的参考。@ 序数合并编号:共享在前、clip 在后 — `@image_1` 在每段都指同一位演员。
...
2. 把演员表作为资产参考挂到节点上 — 每段里就是 `@image_1`、`@image_2`……
...
**Q:共享参考和段内参考的编号会打架吗?**  
A:不会。合并池共享在前,`@image_1` 在每段都是第一个共享参考;段内参考顺延编号。
