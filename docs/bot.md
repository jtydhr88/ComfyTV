...
| [Codex](https://developers.openai.com/codex) | `npm install -g @openai/codex` | `codex login` | not yet |
| [Qwen Code](https://qwenlm.github.io/qwen-code-docs/) | official install script (see its docs) | run `qwen`, then `/auth` | not yet |

Prerequisites:

1. Install at least one agent CLI and sign in once. When both Claude Code and Codex are present, Codex is used first.
