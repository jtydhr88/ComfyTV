...
The bot does not talk to any model API directly and ComfyTV never stores a key. Instead it drives an **agent CLI already installed on your machine**, using whatever login that CLI has. The provider layer is pluggable; two providers ship today:
