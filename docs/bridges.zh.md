...
反方向也有:5 个出桥节点把 ComfyTV 快照转回 ComfyUI 原生类型,让任何插件都能消费 ComfyTV 流水线的产物:
...
`← ComfyTV Mask` 把图片的 alpha 通道读成 mask——把 Cutout 或画笔的结果喂给原生 inpaint 节点时很好用。
...
### 往返:ComfyTV 结果 → 原生插件 → 再回来
```
[Image Stage] → [← ComfyTV Image] ─IMAGE─→ [任何原生插件] ─IMAGE─→ [→ ComfyTV Image] → …
```
