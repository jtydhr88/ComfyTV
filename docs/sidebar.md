...
- **Skills**: manage the [Agent Skills](skills.md) served to agents — a global **Enable Agent Skills** switch, a per-skill enable toggle, **import** of a skill `.zip`, and delete for user-installed skills. Built-in skills can only be disabled, not deleted.
...
- **`@image_N`** — reference the stage's N-th image inline, counting from 1 ("make @image_1's character wear @image_2's outfit"). The media strip above the prompt shows every image the stage will send — wired inputs and pinned assets alike — with its position, and chips share the same color, so you can see at a glance which image is which. Drag the strip to reorder; the tokens in the prompt renumber themselves, and a token whose image was removed is dropped.
