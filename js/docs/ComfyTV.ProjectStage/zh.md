...
- Project 节点提供 `project_id`，让所有 stage 写入同一个项目存储空间。
...
**常见误区**：以为改项目名会移动旧资产——已保存的快照仍绑定原来的 `project_id`，切换项目应通过 UI 项目选择器而不是只改字符串。
...
A：不会。快照存在 ComfyTV 项目存储里；重载 workflow 后，只要 `project_id` 一致，资产仍在。换机器或重装需自行备份 ComfyUI 数据目录。
