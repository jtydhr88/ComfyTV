<template>
  ...
  >
    ...
    />
    <PanelSection :title="$t('pictor.layers')" storage-key="pictor.layerPanelCollapsed">
      <template #actions>
        <button
          v-if="assetPicker"
          type="button"
          :class="miniBtnClass"
          :title="$t('pictor.addFromLibrary')"
    ...
    <div class="ctv:flex ctv:items-center ctv:gap-1 ctv:px-2 ctv:py-1 ctv:text-[10px] ctv:text-[#9b9b9b]">
      <span>{{ $t('pictor.lockLabel') }}</span>
      <button
        type="button"
        :class="[miniBtnClass, (active as any)?.lockAlpha ? 'ctv:text-[#1473e6]' : '']"
        :disabled="!active || active.kind !== 'raster'"
        :title="$t((active as any)?.lockAlpha ? 'pictor.unlockAlpha' : 'pictor.lockAlpha')"
      ...
      <button
        ...
        :disabled="!active"
        :title="$t(active?.locks.content ? 'pictor.unlockLayer' : 'pictor.lockLayer')"
      ...
      <button
        ...
        :disabled="!active"
        :title="$t(active?.locks.position ? 'pictor.unlockPosition' : 'pictor.lockPosition')"
      ...
      <button
        ...
        :disabled="!active"
        :title="$t(active?.locks.content && active?.locks.position ? 'pictor.unlockAll' : 'pictor.lockAll')"
    ...
    >
      ...
      >
        <button
          ...
          :title="$t(row.node.visible ? 'pictor.hideLayer' : 'pictor.showLayer')"
    ...
    <div v-if="active" class="ctv:flex ctv:flex-wrap ctv:items-center ctv:gap-0.5 ctv:px-1.5 ctv:py-0.5">
      <button type="button" :class="miniBtnClass" :title="$t('pictor.moveUp')" @click="editor.moveLayer(active.id, 1)">
        <IconChevronUp class="ctv:size-3.5" />
      </button>
      <button type="button" :class="miniBtnClass" :title="$t('pictor.moveDown')" @click="editor.moveLayer(active.id, -1)">
        <IconChevronDownArrange class="ctv:size-3.5" />
      </button>
      <button type="button" :class="miniBtnClass" :title="$t('pictor.duplicateLayer')" @click="editor.duplicateLayer(active.id)">
        <IconCopy class="ctv:size-3.5" />
      </button>
      <button type="button" :class="miniBtnClass" :title="$t('pictor.groupLayers')" @click="editor.groupActiveLayer()">
      ...
      <button
        ...
        type="button"
        :class="miniBtnClass"
        :title="$t('pictor.ungroupLayers')"
        @click="editor.ungroupActiveLayer()"
      >
        <IconFolderMinus class="ctv:size-3.5" />
      </button>
      <button type="button" :class="miniBtnClass" :title="$t('pictor.mergeDown')" @click="editor.mergeDown(active.id)">
        <IconArrowDownToLine class="ctv:size-3.5" />
      </button>
      <template v-if="active.kind === 'raster'">
        <button type="button" :class="miniBtnClass" :title="$t('pictor.cropToContent')" @click="editor.cropToContent(active.id)">
          <IconCrop class="ctv:size-3.5" />
        </button>
        <button type="button" :class="miniBtnClass" :title="$t('pictor.layerToCanvasSize')" @click="editor.layerToCanvasSize(active.id)">
      ...
      <button type="button" :class="miniBtnClass" :title="$t('pictor.flattenImage')" @click="editor.flattenImage()">
    ...
    <template v-if="active && fxCapable">
      <div class="ctv:flex ctv:items-center ctv:gap-1 ctv:px-2 ctv:pt-1 ctv:text-[10px] ctv:text-[#9b9b9b]">
        ...
        <select :class="presetSelectClass" class="ctv:w-32!" @change="onAddFx">
          <option value="" selected>{{ $t('pictor.fxAdd') }}</option>
          <option v-for="op in LAYER_FX_OPS" :key="op" :value="op">{{ $t(`pictor.fx_${op}`) }}</option>
        </select>
      </div>
      <div v-for="(f, fi) in activeFx" :key="f.id" class="ctv:px-2 ctv:pt-0.5">
        ...
        <template v-for="def in LAYER_FX_DEFS[f.op]" :key="def.key">
          <div v-if="def.color" class="ctv:flex ctv:items-center ctv:gap-1.5 ctv:pt-0.5">
            <span :class="paramLabelClass">{{ $t(`pictor.fxParam_${def.key}`) }}</span>
            <input
              type="color"
          ...
          <FxSlider
            ...
            :label="$t(`pictor.fxParam_${def.key}`)"
            :min="def.min"
            :max="def.max"
    ...
    <template v-if="active">
      <template v-if="active.kind === 'adjustment'">
        ...
        <div class="ctv:flex ctv:items-center ctv:gap-1.5 ctv:px-2 ctv:pt-1">
          <span :class="paramLabelClass">{{ $t('pictor.adjustmentOp') }}</span>
      ...
      <template v-if="active.kind === 'vector'">
        <div class="ctv:mx-2 ctv:border-t ctv:border-[#3d3d3d]" />
        <div class="ctv:flex ctv:items-center ctv:gap-1.5 ctv:px-2 ctv:pt-1">
          <span :class="paramLabelClass">{{ $t('pictor.shapeFill') }}</span>
          <input
            type="checkbox"
        ...
        <div class="ctv:flex ctv:items-center ctv:gap-1.5 ctv:px-2 ctv:pt-1">
          <span :class="paramLabelClass">{{ $t('pictor.shapeStroke') }}</span>
          <input
            type="checkbox"
            class="ctv:accent-[#1473e6]"
      ...
      <template v-if="active.kind === 'fill'">
        <div class="ctv:mx-2 ctv:border-t ctv:border-[#3d3d3d]" />
        <div class="ctv:flex ctv:items-center ctv:gap-1.5 ctv:px-2 ctv:pt-1">
          <span :class="paramLabelClass">{{ $t('pictor.fillType') }}</span>
          <div class="ctv:min-w-0 ctv:flex-1">
            <ComfyTVSelect
        ...
        <div class="ctv:flex ctv:items-center ctv:gap-1.5 ctv:px-2 ctv:pt-1">
          <span :class="paramLabelClass">{{ $t('pictor.fillColors') }}</span>
        ...
        <div v-if="active.fill.type === 'linear'" class="ctv:flex ctv:items-center ctv:gap-1.5 ctv:px-2 ctv:pt-1">
          <span :class="paramLabelClass">{{ $t('pictor.fillAngle') }}</span>
          <input
        ...
        <div v-if="active.fill.type === 'radial'" class="ctv:flex ctv:items-center ctv:gap-1.5 ctv:px-2 ctv:pt-1">
          <span :class="paramLabelClass">{{ $t('pictor.fillRadius') }}</span>
          <input
    ...
    <div class="ctv:flex ctv:items-center ctv:gap-1 ctv:px-2 ctv:py-1 ctv:text-[10px] ctv:text-[#9b9b9b]">
      <span>{{ $t('pictor.sectionCanvas') }}</span>
      <input
    ...
    <div class="ctv:px-2 ctv:pb-1">
      <select
        ...
        :title="$t('pictor.canvasPresetHint')"
        @change="onCanvasPreset"
      >
        <option value="" selected>{{ $t('pictor.canvasPreset') }}</option>
        <optgroup
    ...
    >
      <div class="ctv:flex ctv:items-center ctv:justify-between ctv:pb-0.5">
        <span class="ctv:text-[10px] ctv:uppercase ctv:tracking-wide ctv:text-[#d6d6d6]">
          {{ $t(`pictor.filter_${editor.filterSession.value.op}`) }}
        </span>
        <div class="ctv:flex ctv:items-center ctv:gap-0.5">
          <button type="button" :class="miniBtnClass" :title="$t('pictor.filterApply')" @click="editor.applyFilter()">
            <IconCheck class="ctv:size-3.5" />
          </button>
          <button type="button" :class="miniBtnClass" :title="$t('pictor.filterCancel')" @click="editor.cancelFilter()">
      ...
      <FxSlider
        ...
        :label="$t(`pictor.filterParam_${def.key}`)"
        :min="def.min"
        :max="def.max"
        :step="def.step ?? (def.max - def.min) / 200"
        :decimals="def.max > 10 ? 0 : 2"
    ...
    >
      ...
      <button type="button" :class="menuItemClass" @click="maskMenuAction((id) => editor.toggleMaskEnabled(id))">
        {{ $t(maskMenuNode.mask!.enabled ? 'pictor.maskDisable' : 'pictor.maskEnable') }}
      </button>
...
const PANEL_WIDTH_KEY = 'pictor.layerPanelWidth'
const PANEL_MIN = 192
const PANEL_MAX = 560
