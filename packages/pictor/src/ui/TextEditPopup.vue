  ...
  >
    <div class="ctv:flex ctv:items-center ctv:gap-1">
      <span class="ctv:flex-1 ctv:text-3xs ctv:uppercase ctv:tracking-wide ctv:text-muted-foreground">
        {{ $t('pictor.sectionText') }}
      </span>
      <button type="button" :class="miniBtnClass" :title="$t('pictor.close')" @click="close">
    ...
    <textarea
      ...
      :value="layer.text"
      rows="2"
      :placeholder="$t('pictor.textPlaceholder')"
    ...
    <div class="ctv:flex ctv:items-center ctv:gap-1.5">
      <span :class="labelClass">{{ $t('pictor.font') }}</span>
    ...
    <div v-if="fontFailed" class="ctv:text-2xs ctv:text-red-400">
      {{ $t('pictor.loadFontFailed') }}
    </div>

    <div class="ctv:flex ctv:items-center ctv:gap-1.5">
      <span :class="labelClass">{{ $t('pictor.fontSize') }}</span>
      <input
      ...
      />
      <span :class="labelClass">{{ $t('pictor.textColor') }}</span>
      <input
        type="color"
    ...
    <div class="ctv:flex ctv:items-center ctv:gap-1.5">
      <span :class="labelClass">{{ $t('pictor.letterSpacing') }}</span>
      <input
      ...
      />
      <span :class="labelClass">{{ $t('pictor.lineHeight') }}</span>
      <input
    ...
    <div class="ctv:flex ctv:items-center ctv:gap-1.5">
      <span :class="labelClass">{{ $t('pictor.align') }}</span>
...
import { useTextEditPopup } from './useTextEditPopup'
...
const ALIGNS = [
  { value: 'left' as const, labelKey: 'pictor.alignLeft', icon: IconAlignLeft },
  { value: 'center' as const, labelKey: 'pictor.alignCenter', icon: IconAlignCenter },
  { value: 'right' as const, labelKey: 'pictor.alignRight', icon: IconAlignRight },
]
