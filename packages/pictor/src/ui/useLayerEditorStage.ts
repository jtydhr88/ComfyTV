...
export function useLayerEditorStage(opts: UseLayerEditorStageOptions) {
  ...
  async function uploadDirty(): Promise<void> {
    ...
    try {
      ...
      for (const job of jobs) {
        ...
        const res = await host.uploadBlob(blob, { subfolder: storage.subfolder, filename: `pictor-layer-${instanceId}-${job.contentId}.png` })
      ...
      if (f && !content.get(f.contentId)?.uploadedUrl) {
        ...
        if (entry) {
          ...
          const res = await host.uploadBlob(blob, { subfolder: storage.subfolder, filename: `pictor-float-${instanceId}-${f.contentId}.png` })
      ...
      toastError(t('pictor.uploadFailed'))
    } finally {
      uploading = false
  ...
  async function runCapture(): Promise<void> {
    ...
    try {
      ...
      const url = await host.uploadCanvas(snapshot, { subfolder: storage.subfolder, filenamePrefix: `pictor-cap-${instanceId}` })
      ...
      toastError(t('pictor.captureFailed'))
    }
  }
  ...
  async function captureBatch(): Promise<void> {
    ...
    try {
      editor.render()
      const commit = storage.beginCapture()
      const compositeUrl = await host.uploadCanvas(flattenComposite(), { subfolder: storage.subfolder, filenamePrefix: `pictor-cap-${instanceId}` })
      ...
      try {
        for (let i = 0; i < children.length; i++) {
          ...
          editor.render()
          const url = await host.uploadCanvas(flattenComposite(), { subfolder: storage.subfolder, filenamePrefix: `pictor-layer-${instanceId}` })
  ...
  async function exportPsd(): Promise<void> {
    ...
    try {
      host.download(`pictor-layers-${Date.now()}.psd`, await writePsdBlob())
    } catch (e) {
      console.warn('[pictor] PSD export failed', e)
      toastError(t('pictor.exportPsdFailed'))
    } finally {
  ...
  async function exportPsdToLibrary(): Promise<void> {
    ...
    try {
      ...
      await host.saveToLibrary({
        blob,
        filename: `pictor-layers-${Date.now()}.psd`,
        mime: PSD_MIME,
        width: doc.width,
        height: doc.height,
        preview: flattenComposite(),
      })
      toastInfo(t('pictor.exportPsdAssetDone'))
    } catch (e) {
      console.warn('[pictor] PSD asset export failed', e)
      toastError(t('pictor.exportPsdFailed'))
    } finally {
      exportingPsd.value = false
      requestRender()
  ...
  async function importPsdBuffer(buffer: ArrayBuffer, sourceName: string): Promise<void> {
    ...
    if (!result.nodes.length) {
      toastError(t('pictor.importPsdEmpty'))
      return
    }
  ...
  async function runPsdImport(load: () => Promise<{ buffer: ArrayBuffer; name: string }>): Promise<void> {
    ...
    try {
      ...
      console.warn('[pictor] PSD import failed', e)
      toastError(t('pictor.importPsdFailed'))
    } finally {
  ...
  function addMedia(media: PictorMediaInput): Promise<void> {
  ...
  async function addImageFromUrl(url: string, name: string): Promise<void> {
    try {
      ...
      toastError(t('pictor.loadImageFailed'))
    }
  }
  ...
  async function hydrate(): Promise<void> {
    try {
      ...
      console.warn('[pictor] hydrate failed', e)
    }
  }
