...
import type { PictorMediaInput } from '../host'
import {
  ADJUST_OPS,
  ADJUST_PARAM_DEFS,
  LAYER_MODES,
...
export function useLayerListPanel(editor: LayerEditorController) {
  ...
  const blendOptions = computed(() =>
    (Object.keys(LAYER_MODES) as BlendFn[]).map((mode) => ({
      label: t(`pictor.blend.${BLEND_KEYS[mode] ?? mode}`),
      value: mode,
    })),
  )

  const adjustOptions = computed(() =>
    ADJUST_OPS.map((op) => ({ label: t(`pictor.adjOp.${op}`), value: op })),
  )
  ...
  function onMediaPicked(media: PictorMediaInput): void {
  ...
  const fillTypeOptions = computed(() => [
    { label: t('pictor.fillSolid'), value: 'solid' },
    { label: t('pictor.fillLinear'), value: 'linear' },
    { label: t('pictor.fillRadial'), value: 'radial' },
  ])
