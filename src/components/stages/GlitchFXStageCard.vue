<template>
  <FxCardShell :node="node">
    ...
    <div
      ...
      @pointerup.stop
    >
      <FxSlider v-model="chance" label="Frequency" :min="0" :max="1" :step="0.01" :reset-to="0.5" />
      <FxSlider v-model="blockH" label="Block Height" :min="0.01" :max="1" :step="0.01" :reset-to="0.2" />
      <FxSlider v-model="shift" label="Shift" :min="0.01" :max="1" :step="0.01" :reset-to="0.2" />
      <FxSlider v-model="colorIntensity" label="Color Damage" :min="0" :max="4" :step="1" :decimals="0" :reset-to="3" />
...
const chance = useNumWidget(props.node, 'chance', 0.5)
const blockH = useNumWidget(props.node, 'block_h', 0.2)
const shift = useNumWidget(props.node, 'shift', 0.2)
const colorIntensity = useNumWidget(props.node, 'color_intensity', 3)
...
const preview = useFxClipPreview({
  nodeId: 'ComfyTV.GlitchFXStage',
  getParams: () => ({
    chance: chance.value,
    block_h: blockH.value,
    shift: shift.value,
    color_intensity: colorIntensity.value,
    seed: seed.value,
  }),
