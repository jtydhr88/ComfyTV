<template>
  <FxCardShell :node="node">
    ...
    <div
      ...
      @pointerup.stop
    >
      <FxSlider v-model="segments" label="Segments" :min="1" :max="64" :step="1" :decimals="0" :reset-to="6" />
      <FxSlider v-model="angle" label="Angle" :min="-180" :max="180" :step="1" :decimals="0" :reset-to="0" />
      <FxSlider v-model="sourceAngle" label="Source Angle" :min="-180" :max="180" :step="1" :decimals="0" :reset-to="0" />
      <FxSlider v-model="centerX" label="Center X" :min="0" :max="1" :step="0.005" :reset-to="0.5" />
      <FxSlider v-model="centerY" label="Center Y" :min="0" :max="1" :step="0.005" :reset-to="0.5" />
...
const segments = useNumWidget(props.node, 'segments', 6)
const angle = useNumWidget(props.node, 'angle', 0)
const sourceAngle = useNumWidget(props.node, 'source_angle', 0)
const centerX = useNumWidget(props.node, 'center_x', 0.5)
const centerY = useNumWidget(props.node, 'center_y', 0.5)
...
const preview = useFxClipPreview({
  nodeId: 'ComfyTV.KaleidoscopeStage',
  getParams: () => ({
    segments: segments.value,
    angle: angle.value,
    source_angle: sourceAngle.value,
