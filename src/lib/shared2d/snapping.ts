...
export interface SnapResult { rect: Rect; guides: Guide[] }
...
export function buildSnapTargets(
  otherRects: Rect[], bounds?: { w: number; h: number }, extras?: SnapExtras,
): SnapTargets {
  const bw = bounds?.w ?? 1
  const bh = bounds?.h ?? 1
  const xs = [0, bw / 2, bw]
...
interface EqCandidate { pos: number; guide: Guide }

function overlaps(a0: number, a1: number, b0: number, b1: number): boolean {
  return a0 < b1 && a1 > b0
}

function eqCandidatesAxis(rect: Rect, others: Rect[], axis: 'x' | 'y'): EqCandidate[] {
...
export function applySnap(
  mode: string, rect: Rect, targets: SnapTargets, opts: SnapOpts,
): SnapResult {
  let { x, y, w, h } = rect
  const guides: Guide[] = []
