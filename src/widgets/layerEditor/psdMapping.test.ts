...
import { LAYER_MODES, type BlendFn } from './engine'
...
describe('blend mode mapping', () => {
  it('maps every engine blend mode to a psd mode', () => {
    for (const blend of Object.keys(LAYER_MODES) as BlendFn[]) {
      expect(PSD_BLEND_MODES[blend], blend).toBeTruthy()
    }
  })
...
describe('adjustment mapping', () => {
  it('round-trips brightness-contrast', () => {
    const psd = adjustmentToPsd({ op: 'brightness-contrast', params: { brightness: 0.5, contrast: -0.4 } })!
    expect(psd).toMatchObject({ type: 'brightness/contrast', brightness: 75, contrast: -20 })
  ...
  it('round-trips hue-saturation', () => {
    const psd = adjustmentToPsd({ op: 'hue-saturation', params: { hue: 90, saturation: -0.5, lightness: 0.2 } })!
    const back = adjustmentFromPsd(psd)!
    expect(back.params).toMatchObject({ hue: 90, saturation: -0.5, lightness: 0.2 })
  })

  it('round-trips levels', () => {
    const psd = adjustmentToPsd({
      op: 'levels',
      params: { inBlack: 0.1, inWhite: 0.9, gamma: 2.2, outBlack: 0.05, outWhite: 0.95 },
    })!
    const back = adjustmentFromPsd(psd)!
  ...
  it('round-trips curves via channel strings', () => {
    ...
    expect(curveStringToPsd(back.curves!.master)).toEqual(curveStringToPsd('[[0,0],[1,1]]'))
    expect(curveStringToPsd(back.curves!.red)).toEqual(curveStringToPsd('[[0,0.1],[1,0.9]]'))
  })
  ...
  it('round-trips exposure, color-balance, vibrance, posterize, threshold, invert', () => {
    for (const [op, params] of [
      ...
      expect(psd, op).toBeTruthy()
      const back = adjustmentFromPsd(psd)!
      expect(back.op, op).toBe(op)
      for (const [k, v] of Object.entries(params)) {
        expect(back.params[k], `${op}.${k}`).toBeCloseTo(v, 1)
      }
    }
  })

  it('maps temperature to photo filter and back approximately', () => {
    const warm = adjustmentToPsd({ op: 'temperature', params: { temperature: 4000, mix: 1 } })!
    ...
    const cool = adjustmentToPsd({ op: 'temperature', params: { temperature: 9000, mix: 1 } })!
  ...
  it('returns null for unsupported psd adjustments', () => {
    ...
    expect(adjustmentToPsd({ op: 'bogus' as never, params: {} })).toBeUndefined()
  })
})
...
describe('fill mapping', () => {
  it('round-trips solid fills', () => {
    ...
    expect(vectorContentToFill(content)).toEqual({ type: 'solid', color: '#ff8800' })
  })
  ...
  it('round-trips linear gradients', () => {
    ...
    expect(content).toMatchObject({ type: 'solid', style: 'linear', angle: -45 })
    ...
    if (back.type === 'linear') {
      ...
      expect(back.stops[1]).toMatchObject({ offset: 1, color: '#ffffff', alpha: 0.5 })
    }
  })
  ...
  it('rejects unsupported content', () => {
    ...
    expect(vectorContentToFill({ type: 'noise', name: 'n', roughness: 1, min: [], max: [] } as never)).toBeNull()
  })
})
...
describe('path mapping', () => {
  it('round-trips a rect path', () => {
    ...
    expect(back.strokes[0].anchors.map((a) => a.pos)).toEqual(path.strokes[0].anchors.map((a) => a.pos))
    expect(back.strokes[0].anchors.map((a) => a.type)).toEqual(path.strokes[0].anchors.map((a) => a.type))
  })

  it('applies point mappers on export', () => {
    const path = rectPath(0, 0, 10, 10)
    const psd = pathToBezierPaths(path, 'even-odd', (pt) => ({ x: pt.x + 5, y: pt.y + 7 }))
  ...
  it('skips malformed knots on import', () => {
    const back = bezierPathsToPath([
      { open: true, knots: [{ linked: true, points: [1, 2] }], fillRule: 'non-zero' },
    ])
