...
export const PSD_BLEND_MODES: Record<BlendFn, NonNullable<Layer['blendMode']>> = {
  normal: 'normal',
  multiply: 'multiply',
  screen: 'screen',
  overlay: 'overlay',
...
export function psdColorToHex(color: Color | undefined): string {
  ...
  const to255 = (v: number | undefined): number => {
    ...
    return Math.round(Math.max(0, Math.min(255, n <= 1 && n > 0 && !Number.isInteger(n) ? n * 255 : n)))
  }
...
export function curveStringToPsd(raw: string | undefined): CurvesAdjustmentChannel | undefined {
...
export function psdCurveToString(channel: CurvesAdjustmentChannel | undefined): string {
...
export function adjustmentToPsd(node: Pick<AdjustmentData, 'op' | 'params' | 'curves'>): AdjustmentLayer | undefined {
  const p = node.params ?? {}
  switch (node.op) {
    case 'invert':
      return { type: 'invert' }
    ...
    case 'color-balance': {
      const triple = (prefix: string) => ({
        ...
        magentaGreen: Math.round(clamp(num(p[`${prefix}G`], 0) * 100, -100, 100)),
    ...
    case 'vibrance':
      return { type: 'vibrance', vibrance: Math.round(clamp(num(p.amount, 0) * 50, -100, 100)), saturation: 0 }
    case 'posterize':
      return { type: 'posterize', levels: Math.round(clamp(num(p.levels, 4), 2, 255)) }
    case 'threshold':
      return { type: 'threshold', level: Math.round(clamp(num(p.level, 0.5) * 255, 1, 255)) }
    case 'temperature': {
      const temp = num(p.temperature, 6500)
      const mix = clamp(num(p.mix, 1), 0, 1)
      const density = Math.round(clamp((Math.abs(temp - 6500) / 5500) * 100 * mix, 0, 100))
      if (density === 0) return { type: 'photo filter', color: WARMING, density: 1, preserveLuminosity: true }
      return {
        type: 'photo filter',
...
export function adjustmentFromPsd(adj: AdjustmentLayer): ImportedAdjustment | null {
  switch (adj.type) {
    case 'invert':
    ...
    case 'color balance': {
      const from = (v: { cyanRed: number; magentaGreen: number; yellowBlue: number } | undefined, prefix: string) => ({
    ...
    case 'vibrance':
      return { op: 'vibrance', params: { amount: clamp(num(adj.vibrance, 0) / 50, -2, 2) } }
    case 'posterize':
      return { op: 'posterize', params: { levels: clamp(Math.round(num(adj.levels, 4)), 2, 32) } }
    case 'threshold':
      return { op: 'threshold', params: { level: clamp(num(adj.level, 128) / 255, 0, 1) } }
    case 'photo filter': {
      ...
      return {
        op: 'temperature',
        params: { temperature: clamp(warm ? 6500 - offset : 6500 + offset, 1000, 12000), mix: 1 },
      }
    }
    default:
      return null
...
function toColorStops(stops: GradientStop[]) {
  return stops.map((s) => ({ color: hexToPsdColor(s.color), location: clamp(s.offset, 0, 1), midpoint: 0.5 }))
}

function toOpacityStops(stops: GradientStop[]) {
  return stops.map((s) => ({ opacity: s.alpha === undefined ? 1 : clamp(s.alpha, 0, 1), location: clamp(s.offset, 0, 1), midpoint: 0.5 }))
}
...
export function fillToVectorContent(spec: FillSpec): VectorContent {
  ...
  if (spec.type === 'linear') {
    ...
    return { ...base, style: 'linear', angle: angle > 180 ? angle - 360 : angle, scale: 100 }
  }
  return {
    ...base,
    style: 'radial',
...
export function vectorContentToFill(content: VectorContent | undefined): FillSpec | null {
  ...
  const opacityAt = (location: number): number | undefined => {
    const stop = (content.opacityStops ?? []).find((o) => Math.abs(o.location - location) < 1e-3)
  ...
  const extra = content as VectorContent & { style?: string; angle?: number; scale?: number; offset?: { x: number; y: number } }
...
export function bezierPathsToPath(paths: BezierPath[]): PathData {
  ...
  for (const path of paths) {
    ...
    for (const knot of path.knots) {
      ...
      anchors.push(
        { pos: { x: num(p[0], 0), y: num(p[1], 0) }, type: 'control', selected: false },
        { pos: { x: num(p[2], 0), y: num(p[3], 0) }, type: 'anchor', selected: false },
        { pos: { x: num(p[4], 0), y: num(p[5], 0) }, type: 'control', selected: false }
      )
    }
    if (anchors.length >= 3) {
...
export function alignFromPsd(justification: Justification | undefined): 'left' | 'center' | 'right' {
